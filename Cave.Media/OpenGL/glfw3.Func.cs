...
public static partial class glfw3
{
    /// <summary>
    /// <para>
    /// This function initializes the GLFW library. Before most GLFW functions can be used, GLFW must be initialized, and before an application terminates GLFW
    ...
    /// application exits.
    /// </para>
    /// <para>Additional calls to this function after successful initialization but before termination will return <c>true</c> immediately.</para>
    /// </summary>
    /// <returns><c>true</c> if successful, or <c>false</c> if an error occurred.</returns>
    /// <remarks>
    /// <strong>OSX:</strong> This function will change the current directory of the application to the <c>Contents/Resources</c> subdirectory of the
    ...
    /// is called, you must again call <see cref="Init"/> successfully before you will be able to use most GLFW functions.
    /// </para>
    /// <para>
    ...
    /// <summary>
    /// <para>
    /// This function returns the compile-time generated version string of the GLFW library binary. It describes the version, platform, compiler and any
    /// platform-specific compile-time options. It should not be confused with the OpenGL or OpenGL ES version string, queried with <c>glGetString</c>.
    /// </para>
    /// <para>
    /// <strong>Do not use the version string</strong> to parse the GLFW library version. The <see cref="GetVersion(out int, out int, out int)"/> function
    /// provides the version of the running library binary in numerical format.
    /// </para>
    /// </summary>
    /// <returns>The ASCII encoded GLFW version string.</returns>
    /// <remarks>This function may be called before <see cref="Init"/>.</remarks>
    /// <seealso cref="GetVersion(out int, out int, out int)"/>
    public static unsafe UTF8 GetVersionString() => MarshalStruct.ReadUtf8(glfwGetVersionString());
    ...
    /// <summary>
    /// This function returns an array of handles for all currently connected monitors. The primary monitor is always first in the returned array. If no
    /// monitors were found, this function returns <c>null</c>.
    /// </summary>
    /// <returns>An array of monitor handles.</returns>
    ...
    /// <summary>This function returns the primary monitor. This is usually the monitor where elements like the task bar or global menu bar are located.</summary>
    ...
    /// <summary>This function returns the position, in screen coordinates, of the upper-left corner of the specified monitor.</summary>
    ...
    /// <para>
    /// Some systems do not provide accurate monitor size information, either because the monitor <a
    /// href="https://en.wikipedia.org/wiki/Extended_display_identification_data">EDID</a> data is incorrect or because the driver does not report it accurately.
    /// </para>
    /// </summary>
    /// <param name="monitor">The monitor to query.</param>
    ...
    /// <remarks>
    /// <strong>Win32:</strong> Calculates the returned physical size from the current resolution and system DPI instead of querying the monitor EDID data.
    /// </remarks>
    ...
    /// <summary>
    /// This function returns a human-readable name, encoded as UTF-8, of the specified monitor. The name typically reflects the make and model of the monitor
    ...
    public static unsafe UTF8 GetMonitorName(Monitor monitor) => MarshalStruct.ReadUtf8(glfwGetMonitorName(monitor.Ptr));
    ...
    /// <summary>
    /// This function sets the monitor configuration callback, or removes the currently set callback. This is called when a monitor is connected to or
    ...
    /// <summary>
    /// This function returns an array of all video modes supported by the specified monitor. The returned array is sorted in ascending order, first by color
    /// bit depth (the sum of all channel depths) and then by resolution area (the product of width and height).
    /// </summary>
    /// <param name="monitor">The monitor to query.</param>
    ...
    /// <summary>
    /// This function returns the current video mode of the specified monitor. If you have created a full screen window for that monitor, the return value will
    ...
    /// <summary>
    /// This function generates a 256-element gamma ramp from the specified exponent and then calls <see cref="SetGammaRamp(Monitor, GammaRamp)"/> with it. The
    ...
    /// <remarks>
    /// Gamma ramp sizes other than 256 are not supported by all platforms or graphics hardware ( <strong>Win32</strong> requires a 256 gamma ramp size).
    /// </remarks>
    ...
    /// <summary>This function resets all window hints to their default values.</summary>
    ...
    /// <summary>
    /// <para>
    /// This function sets hints for the next call to <see cref="CreateWindow"/>. The hints, once set, retain their values until changed by a call to <see
    /// cref="WindowHint(Hint, int)"/> or <see cref="DefaultWindowHints"/>, or until the library is terminated.
    /// </para>
    /// <para>This function does not check whether the specified hint values are valid.</para>
    /// <para>If you set hints to invalid values this will instead be reported by the next call to <see cref="CreateWindow"/>.</para>
    /// </summary>
    ...
    /// <summary>
    /// <para>
    /// This function creates a window and its associated OpenGL or OpenGL ES context. Most of the options controlling how the window and its context should be
    /// created are specified with window hints.
    /// </para>
    /// <para>Successful creation does not change which context is current. Before you can use the newly created context, you need to make it current.</para>
    /// <para>
    ...
    /// size of the window, especially for full screen windows. To query the actual attributes of the created window, framebuffer and context see <see
    /// cref="GetWindowAttrib(Window, WindowAttrib)"/>, <see cref="GetWindowSize(Window, out int, out int)"/> and <see cref="GetFramebufferSize(Window, out int,
    /// out int)"/>.
    /// </para>
    /// <para>
    ...
    /// Unless you have a way for the user to choose a specific monitor, it is recommended that you pick the primary monitor.
    /// </para>
    /// <para>
    /// For full screen windows, the specified size becomes the resolution of the window's <em>desired video mode</em>. As long as a full screen window is not
    /// iconified, the supported video mode most closely matching the desired video mode is set for the specified monitor.
    /// </para>
    /// <para>
    ...
    /// invisible using the <see cref="Hint.Visible"/> hint, set its position and then show it.
    /// </para>
    ...
    /// <param name="share">The window whose context to share resources with, or <c>null</c> to not share resources.</param>
    /// <returns>The handle of the created window, or <c>null</c> if an error occurred.</returns>
    /// <remarks>
    /// <para><strong>Win32:</strong> Window creation will fail if the Microsoft GDI software OpenGL implementation is the only one available.</para>
    /// <para>
    /// <strong>Win32:</strong> If the executable has an icon resource named <c>GLFW_ICON</c>, it will be set as the initial icon for the window. If no such
    /// icon is present, the <c>IDI_WINLOGO</c> icon will be used instead. To set a different icon, use <see cref="SetWindowIcon(Window, Image)"/>.
    /// </para>
    ...
    /// </para>
    /// <para>
    /// <strong>OSX:</strong> The first time a window is created the menu bar is populated with common commands like Hide, Quit and About. The About entry opens
    ...
    /// <para>
    /// <strong>X11:</strong> Due to the asynchronous nature of X11, it may take a moment for a window to reach its requested state.This means you may not be
    /// able to query the final size, position or other attributes directly after window creation.
    /// </para>
    /// </remarks>
    ...
    /// <summary>
    /// This function sets the value of the close flag of the specified window. This can be used to override the user's attempt to close the window, or to
    /// signal that it should be closed.
    /// </summary>
    ...
    /// <summary>This function sets the window title, encoded as UTF-8, of the specified window.</summary>
    /// <param name="window">The window whose title to change.</param>
    /// <param name="title">The UTF-8 encoded window title.</param>
    /// <remarks><strong>OSX:</strong> The window title will not be updated until the next time you process events.</remarks>
    ...
    /// <summary>This function retrieves the position, in screen coordinates, of the upper-left corner of the client area of the specified window.</summary>
    /// <param name="window">The window to query.</param>
    ...
    /// <summary>
    /// <para>
    /// This function sets the position, in screen coordinates, of the upper-left corner of the client area of the specified windowed mode window. If the window
    /// is a full screen window, this function does nothing.
    /// </para>
    /// <para>
    /// <strong>Do not use this function</strong> to move an already visible window unless you have very good reasons for doing so, as it will confuse and annoy
    /// the user.
    /// </para>
    ...
    /// <summary>
    /// This function retrieves the size, in screen coordinates, of the client area of the specified window.If you wish to retrieve the size of the framebuffer
    /// of the window in pixels, see <see cref="GetFramebufferSize(Window, out int, out int)"/>.
    /// </summary>
    ...
    /// <summary>
    /// <para>
    /// This function sets the size limits of the client area of the specified window. If the window is full screen, the size limits only take effect once it is
    ...
    /// <param name="minwidth">The minimum width, in screen coordinates, of the client area, or <see cref="DontCare"/>.</param>
    /// <param name="minheight">The minimum height, in screen coordinates, of the client area, or <see cref="DontCare"/>.</param>
    /// <param name="maxwidth">The maximum width, in screen coordinates, of the client area, or <see cref="DontCare"/>.</param>
    /// <param name="maxheight">The maximum height, in screen coordinates, of the client area, or <see cref="DontCare"/>.</param>
    ...
    /// <summary>
    /// <para>
    /// This function sets the required aspect ratio of the client area of the specified window. If the window is full screen, the aspect ratio only takes
    /// effect once it is made windowed. If the window is not resizable, this function does nothing.
    /// </para>
    /// <para>
    ...
    /// </para>
    /// <para>If the numerator and denominator is set to <see cref="DontCare"/> then the aspect ratio limit is disabled.</para>
    ...
    /// <summary>
    /// <para>This function sets the size, in screen coordinates, of the client area of the specified window.</para>
    /// <para>
    ...
    /// </para>
    /// <para>If you wish to update the refresh rate of the desired video mode in addition to its resolution, see @ref glfwSetWindowMonitor.</para>
    ...
    /// <summary>
    /// This function retrieves the size, in pixels, of the framebuffer of the specified window. If you wish to retrieve the size of the window in screen
    /// coordinates, see <see cref="GetWindowSize(Window, out int, out int)"/>.
    /// </summary>
    ...
    /// <summary>
    /// <para>
    /// This function retrieves the size, in screen coordinates, of each edge of the frame of the specified window. This size includes the title bar, if the
    /// window has one. The size of the frame may vary depending on the window - related hints used to create it.
    /// </para>
    /// <para>
    ...
    /// <summary>
    /// <para>
    /// This function restores the specified window if it was previously iconified (minimized) or maximized.If the window is already restored, this function
    /// does nothing.
    /// </para>
    ...
    /// <summary>
    /// This function makes the specified window visible if it was previously hidden. If the window is already visible or is in full screen mode, this function
    /// does nothing.
    /// </summary>
    ...
    /// <para>
    /// By default, both windowed and full screen mode windows are focused when initially created. Set the <see cref="Hint.Focused"/> hint to disable this behavior.
    /// </para>
    /// <para>
    /// <strong>Do not use this function</strong> to steal focus from other applications unless you are certain that is what the user wants. Focus stealing can
    ...
    /// <summary>This function returns the handle of the monitor that the specified window is in full screen on.</summary>
    /// <param name="window">The window to query.</param>
    ...
    /// <summary>
    /// <para>This function sets the monitor that the window uses for full screen mode or, if the monitor is <see cref="Monitor.None"/>, makes it windowed mode.</para>
    /// <para>
    ...
    /// </para>
    /// <para>
    /// When the monitor is <see cref="Monitor.None"/>, the position, width and height are used to place the window client area. The refresh rate is ignored
    /// when no monitor is specified.
    /// </para>
    /// <para>
    /// If you only wish to update the resolution of a full screen window or the size of a windowed mode window, see <see cref="SetWindowSize(Window, int, int)"/>"/&gt;.
    /// </para>
    /// <para>
    ...
    /// <summary>This function returns the value of an attribute of the specified window or its OpenGL or OpenGL ES context.</summary>
    /// <param name="window">The window to query.</param>
    ...
    /// <remarks>Framebuffer related hints are not window attributes.</remarks>
    ...
    /// <summary>
    /// This function sets the user-defined pointer of the specified window. The current value is retained until the window is destroyed. The initial value is <c>IntPtr.Zero</c>.
    /// </summary>
    ...
    /// <summary>This function sets the user-defined pointer of the specified window. The initial value is <c>IntPtr.Zero</c>.</summary>
    ...
    /// <summary>
    /// This function sets the position callback of the specified window, which is called when the window is moved. The callback is provided with the screen
    ...
    /// <summary>
    /// This function sets the size callback of the specified window, which is called when the window is resized. The callback is provided with the size, in
    /// screen coordinates, of the client area of the window.
    /// </summary>
    /// <param name="window">The window whose callback to set.</param>
    /// <param name="callback">The new callback, or <c>null</c> to remove the currently set callback.</param>
    ...
    /// <summary>
    /// <para>
    /// This function sets the close callback of the specified window, which is called when the user attempts to close the window, for example by clicking the
    /// close widget in the title bar.
    /// </para>
    /// <para>The close flag is set before this callback is called, but you can modify it at any time with <see cref="SetWindowShouldClose(Window, bool)"/>.</para>
    /// <para>The close callback is not triggered by <see cref="DestroyWindow(Window)"/>.</para>
    /// </summary>
    /// <param name="window">The window whose callback to set.</param>
    /// <param name="callback">The new callback, or <c>null</c> to remove the currently set callback.</param>
    /// <remarks><strong>OSX:</strong> Selecting Quit from the application menu will trigger the close callback for all windows.</remarks>
    ...
    /// <summary>
    /// <para>
    /// This function sets the refresh callback of the specified window, which is called when the client area of the window needs to be redrawn, for example if
    ...
    /// <para>
    /// After the focus callback is called for a window that lost input focus, synthetic key and mouse button release events will be generated for all such that
    /// had been pressed. For more information, see <see cref="SetKeyCallback(Window, KeyFunc)"/> and <see cref="SetMouseButtonCallback(Window, MouseButtonFunc)"/>.
    /// </para>
    /// </summary>
    /// <param name="window">The window whose callback to set.</param>
    /// <param name="callback">The new callback, or <c>null</c> to remove the currently set callback.</param>
    ...
    /// <summary>This function sets the iconification callback of the specified window, which is called when the window is iconified or restored.</summary>
    /// <param name="window">The window whose callback to set.</param>
    /// <param name="callback">The new callback, or <c>null</c> to remove the currently set callback.</param>
    ...
    /// behaves exactly like <see cref="PollEvents"/>, i.e. the events in the queue are processed and the function then returns immediately. Processing events
    ...
    /// <summary>
    /// <para>
    /// This function puts the calling thread to sleep until at least one event is available in the event queue, or until the specified timeout is reached. Once
    /// one or more events are available, it behaves exactly like <see cref="PollEvents"/>, i.e. the events in the queue are processed and the function then
    /// returns immediately. Processing events will cause the window and input callbacks associated with those events to be called.
    /// </para>
    /// <para>
    /// Since not all events are associated with callbacks, this function may return without a callback having been called even if you are monitoring all callbacks.
    /// </para>
    ...
    /// <summary>
    /// <para>
    /// This function posts an empty event from the current thread to the event queue, causing <see cref="WaitEvents"/> or <see
    /// cref="WaitEventsTimeout(double)"/> to return.
    /// </para>
    /// <para>
    /// If no windows exist, this function returns immediately. For synchronization of threads in applications that do not create windows, use your threading
    /// library of choice.
    ...
    /// <summary>This function returns the value of an input option for the specified window.</summary>
    /// <param name="window">The window to query.</param>
    ...
    /// <para>If the mode is <see cref="InputMode.Cursor"/>, the value must be one of the following cursor modes:</para>
    /// <list type="bullet">
    /// <item><see cref="CursorMode.Normal"/> makes the cursor visible and behaving normally.</item>
    /// <item>
    /// <see cref="CursorMode.Hidden"/> makes the cursor invisible when it is over the client area of the window but does not restrict the cursor from leaving.
    /// </item>
    /// <item>
    /// <see cref="CursorMode.Disabled"/> hides and grabs the cursor, providing virtual and unlimited cursor movement.This is useful for implementing for
    /// example 3D camera controls.
    /// </item>
    /// </list>
    /// <para>
    /// If the mode is <see cref="InputMode.StickyKeys"/>, the value must be either <c>true</c> to enable sticky keys, or <c>false</c> to disable it. If sticky
    /// keys are enabled, a key press will ensure that <see cref="GetKey(Window, int)"/> returns <see cref="InputState.Press"/> the next time it is called even
    /// if the key had been released before the call. This is useful when you are only interested in whether keys have been pressed but not when or in which order.
    /// </para>
    /// <para>
    /// If the mode is <see cref="InputMode.StickyMouseButton"/>, the value must be either <c>true</c> to enable sticky mouse buttons, or <c>false</c> to
    /// disable it. If sticky mouse buttons are enabled, a mouse button press will ensure that <see cref="GetMouseButton"/> returns <see
    /// cref="InputState.Press"/> the next time it is called even if the mouse button had been released before the call. This is useful when you are only
    /// interested in whether mouse buttons have been pressed but not when or in which order.
    /// </para>
    /// </summary>
    ...
    /// <summary>
    /// <para>This function returns the localized name of the specified printable key. This is intended for displaying key bindings to the user.</para>
    /// <para>
    /// If the key is <see cref="KeyCode.Unknown"/>, the scancode is used instead, otherwise the scancode is ignored. If a non-printable key or (if the key is
    /// <see cref="KeyCode.Unknown"/>) a scancode that maps to a non-printable key is specified, this function returns <c>null</c>.
    /// </para>
    ...
    /// <summary>
    /// <para>
    /// This function returns the last state reported for the specified key to the specified window. The returned state is one of <see cref="InputState.Press"/>
    /// or <see cref="InputState.Release"/>. The higher-level action <see cref="InputState.Repeat"/> is only reported to the key callback.
    /// </para>
    /// <para>
    /// If the <see cref="InputMode.StickyKeys"/> input mode is enabled, this function returns <see cref="InputState.Press"/> the first time you call it for a
    ...
    /// <param name="key">The desired keyboard key. <see cref="KeyCode.Unknown"/> is not a valid key for this function.</param>
    ...
    /// <summary>
    /// <para>
    /// This function returns the last state reported for the specified mouse button to the specified window. The returned state is one of <see
    /// cref="InputState.Press"/> or <see cref="InputState.Release"/>.
    /// </para>
    /// <para>
    /// If the <see cref="InputMode.StickyKeys"/> input mode is enabled, this function <see cref="InputState.Press"/> the first time you call it for a mouse
    ...
    /// </para>
    /// <para>
    /// If the cursor is disabled (with <see cref="CursorMode.Disabled"/>) then the cursor position is unbounded and limited only by the minimum and maximum
    /// values of a <c>double</c>.
    /// </para>
    /// <para>
    /// The coordinate can be converted to their integer equivalents with the <c>floor</c> function. Casting directly to an integer type works for positive
    /// coordinates, but fails for negative ones.
    /// </para>
    /// </summary>
    /// <param name="window">The desired window.</param>
    /// <param name="xpos">Where to store the cursor x-coordinate, relative to the left edge of the client area.</param>
    /// <param name="ypos">Where to store the cursor y-coordinate, relative to the to top edge of the client area.</param>
    ...
    /// <summary>
    /// <para>
    /// This function sets the position, in screen coordinates, of the cursor relative to the upper-left corner of the client area of the specified window. The
    /// window must have input focus. If the window does not have input focus when this function is called, it fails silently.
    /// </para>
    /// <para>
    /// <strong>Do not use this function</strong> to implement things like camera controls. GLFW already provides the <see cref="CursorMode.Disabled"/> cursor
    /// mode that hides the cursor, transparently re-centers it and provides unconstrained cursor motion. See <see cref="SetInputMode(Window, InputMode,
    /// CursorMode)"/> for more information.
    /// </para>
    /// <para>
    /// If the cursor mode is <see cref="CursorMode.Disabled"/> then the cursor position is unconstrained and limited only by the minimum and maximum values of
    /// a <c>double</c>.
    /// </para>
    /// </summary>
    /// <param name="window">The desired window.</param>
    ...
    /// <summary>
    /// <para>
    /// Creates a new custom cursor image that can be set for a window with <see cref="SetCursor(Window, Cursor)"/>. The cursor can be destroyed with <see
    /// cref="DestroyCursor(Cursor)"/>. Any remaining cursors are destroyed by <see cref="Terminate"/>.
    /// </para>
    /// <para>
    ...
    /// <summary>Returns a cursor with a standard shape, that can be set for a window with <see cref="CreateCursor(Image, int, int)"/>.</summary>
    ...
    /// <summary>
    /// This function destroys a cursor previously created with <see cref="CreateCursor(Image, int, int)"/>. Any remaining cursors will be destroyed by <see cref="Terminate"/>.
    /// </summary>
    ...
    /// <summary>
    /// <para>
    /// This function sets the cursor image to be used when the cursor is over the client area of the specified window.The set cursor will only be visible when
    /// the cursor mode of the window is <see cref="CursorMode.Normal"/>.
    /// </para>
    ...
    /// to input text, use the <see cref="SetCharCallback(Window, CharFunc)"/> instead.
    /// </para>
    /// <para>
    ...
    /// events by the fact that the synthetic ones are generated after the focus loss event has been processed, i.e. after the <see
    /// cref="SetWindowFocusCallback(Window, WindowFocusFunc)"/> has been called.
    /// </para>
    /// <para>
    ...
    /// a GLFW key token. Such keys have <c>key</c> set to <see cref="KeyCode.Unknown"/>, their state is not saved and so it cannot be queried with <see
    /// cref="GetKey(Window, int)"/>.
    /// </para>
    ...
    /// <para>
    /// The character callback is intended for Unicode text input. As it deals with characters, it is keyboard layout dependent, whereas <see
    /// cref="SetKeyCallback(Window, KeyFunc)"/> is not. Characters do not map 1:1 to physical keys, as a key may produce zero, one or more characters. If you
    /// want to know whether a specific physical key was pressed or released, see the key callback instead.
    /// </para>
    /// <para>
    ...
    /// input on that platform, for example a Super (Command) key on OS X or Alt key on Windows. There is <see cref="SetCharModsCallback(Window,
    /// CharModsFunc)"/> that receives these events.
    /// </para>
    /// </summary>
    /// <param name="window">The window whose callback to set.</param>
    /// <param name="callback">The new callback, or <c>null</c> to remove the currently set callback.</param>
    ...
    /// <summary>
    /// <para>
    /// This function sets the character with modifiers callback of the specified window, which is called when a Unicode character is input regardless of what
    /// modifier keys are used.
    /// </para>
    /// <para>
    /// The character with modifiers callback is intended for implementing custom Unicode character input. For regular Unicode text input, use <see
    /// cref="SetCharCallback(Window, CharFunc)"/>. Like the character callback, the character with modifiers callback deals with characters and is keyboard
    /// layout dependent. Characters do not map 1:1 to physical keys, as a key may produce zero, one or more characters.If you want to know whether a specific
    /// physical key was pressed or released, see <see cref="SetKeyCallback(Window, KeyFunc)"/> instead.
    /// </para>
    /// </summary>
    /// <param name="window">The window whose callback to set.</param>
    /// <param name="callback">The new callback, or <c>null</c> to remove the currently set callback.</param>
    ...
    /// user-generated events by the fact that the synthetic ones are generated after the focus loss event has been processed, i.e. after <see
    /// cref="SetWindowFocusCallback(Window, WindowFocusFunc)"/> has been called.
    /// </para>
    /// </summary>
    /// <param name="window">The window whose callback to set.</param>
    ...
    /// <summary>
    /// This function sets the cursor position callback of the specified window, which is called when the cursor is moved.The callback is provided with the
    ...
    /// <summary>
    /// This function sets the cursor boundary crossing callback of the specified window, which is called when the cursor enters or leaves the client area of
    /// the window.
    /// </summary>
    /// <param name="window">The window whose callback to set.</param>
    /// <param name="callback">The new callback, or <c>null</c> to remove the currently set callback.</param>
    ...
    /// <summary>
    /// <para>
    /// This function sets the scroll callback of the specified window, which is called when a scrolling device is used, such as a mouse wheel or scrolling area
    /// of a touchpad.
    /// </para>
    ...
    /// <summary>This function sets the file drop callback of the specified window, which is called when one or more dragged files are dropped on the window.</summary>
    /// <param name="window">The window whose callback to set.</param>
    /// <param name="callback">The new callback, or <c>null</c> to remove the currently set callback.</param>
    ...
    /// <para>
    /// Querying a joystick slot with no device present is not an error, but will cause this function to return <c>null</c>. Call <see
    /// cref="JoystickPresent(Joystick)"/> to check device presence.
    /// </para>
    /// </summary>
    /// <param name="joy">The joystick to query.</param>
    /// <returns>An array of axis values, (empty if no joystick is present).</returns>
    ...
    /// </summary>
    /// <param name="joy">The joystick to query.</param>
    /// <returns>An array of button states, or empty if no joystick is present.</returns>
    ...
    public static UTF8 GetJoystickName(Joystick joy) => MarshalStruct.ReadUtf8(glfwGetJoystickName((int)joy));
    ...
    /// <summary>
    /// This function sets the joystick configuration callback, or removes the currently set callback. This is called when a joystick is connected to or
    /// disconnected from the system.
    /// </summary>
    /// <param name="callback">The new callback, or <c>null</c> to remove the currently set callback.</param>
    ...
    /// <summary>This function sets the system clipboard to the specified, UTF-8 encoded string.</summary>
    ...
    /// <summary>
    /// This function returns the contents of the system clipboard, if it contains or is convertible to a UTF-8 encoded string. If the clipboard is empty or if
    /// its contents cannot be converted, <c>null</c> is returned and a <see cref="ErrorCode.FormatUnavailable"/> error is generated.
    /// </summary>
    ...
    public static UTF8 GetClipboardString(Window window) => MarshalStruct.ReadUtf8(glfwGetClipboardString(window.Ptr));
    ...
    /// <summary>
    /// <para>
    /// This function returns the value of the GLFW timer. Unless the timer has been set using <see cref="SetTime(double)"/>, the timer measures time elapsed
    ...
    /// <summary>
    /// This function sets the value of the GLFW timer. It then continues to count up from that value. The value must be a positive finite number less than or
    /// equal to 18446744073.0 which is approximately 584.5 years.
    /// </summary>
    /// <param name="time">The new value, in seconds.</param>
    /// <remarks>
    /// The upper limit of the timer is calculated as <c>floor((2^64 - 1) / 10^9)</c> and is due to implementations storing nanoseconds in 64 bits. The limit
    ...
    /// <summary>This function returns the current value of the raw timer, measured in <c>1/frequency</c> seconds. To get the frequency, call <see cref="GetTimerFrequency"/>.</summary>
    ...
    /// <summary>
    /// <para>
    /// This function makes the OpenGL or OpenGL ES context of the specified window current on the calling thread. A context can only be made current on a
    /// single thread at a time and each thread can have only a single current context at a time.
    /// </para>
    /// <para>
    /// By default, making a context non-current implicitly forces a pipeline flush. On machines that support <c>_KHR_context_flush_control</c>, you can control
    /// whether a context performs this flush by setting the <see cref="Hint.ContextReleaseBehavior"/> window hint.
    /// </para>
    /// <para>
    /// The specified window must have an OpenGL or OpenGL ES context. Specifying a window without a context will generate a <see
    /// cref="ErrorCode.NoWindowContext"/> error.
    /// </para>
    /// </summary>
    /// <param name="window">The window whose context to make current, or <see cref="Window.None"/> to detach the current context.</param>
    ...
    /// <summary>This function returns the window whose OpenGL or OpenGL ES context is current on the calling thread.</summary>
    /// <returns>The window whose context is current, or <see cref="Window.None"/> if no window's context is current.</returns>
    ...
    /// <summary>
    /// <para>
    /// This function swaps the front and back buffers of the specified window when rendering with OpenGL or OpenGL ES. If the swap interval is greater than
    ...
    /// <remarks><strong>EGL:</strong> The context of the specified window must be current on the calling thread.</remarks>
    ...
    /// <summary>
    /// <para>
    /// This function sets the swap interval for the current OpenGL or OpenGL ES context, i.e. the number of screen updates to wait from the time <see
    /// cref="SwapBuffers(Window)"/> was called before swapping the buffers and returning. This is sometimes called <em>vertical synchronization</em>,
    /// <em>vertical retrace synchronization</em> or just <em>vsync</em>.
    /// </para>
    /// <para>
    /// Contexts that support either of the <c>W_EXT_swap_control_tear</c> and <c>GLX_EXT_swap_control_tear</c> extensions also accept negative swap intervals,
    /// which allow the driver to swap even if a frame arrives a little bit late.
    /// </para>
    /// <para>
    /// You can check for the presence of these extensions using <see cref="ExtensionSupported(string)"/>. For more information about swap tearing, see the
    /// extension specifications.
    /// </para>
    /// <para>
    /// A context must be current on the calling thread. Calling this function without a current context will cause a <see cref="ErrorCode.NoCurrentContext"/> error.
    /// </para>
    /// </summary>
    /// <param name="interval">The minimum number of screen updates to wait for until the buffers are swapped by <see cref="SwapBuffers(Window)"/>.</param>
    /// <remarks>
    /// <para>
    /// This function is not called during context creation, leaving the swap interval set to whatever is the default on that platform. This is done because
    ...
    /// </para>
    /// <para>
    /// Some GPU drivers do not honor the requested swap interval, either because of a user setting that overrides the application's request or due to bugs in
    /// the driver.
    /// </para>
    /// </remarks>
    ...
    /// <summary>
    /// <para>
    /// This function returns whether the specified API extension is supported by the current OpenGL or OpenGL ES context. It searches both for client API
    ...
    /// <para>
    /// This function may return a non- <c>null</c> address despite the associated version or extension not being available. Always check the context version or
    ...
    /// <summary>This function returns whether the Vulkan loader has been found. This check is performed by <see cref="Init"/>.</summary>
