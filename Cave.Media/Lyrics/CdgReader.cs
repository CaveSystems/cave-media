...
public class CdgReader
{
    ...
    public bool GetNextPacket(out CdgPacket packet, out TimeSpan time)
    {
        ...
        if (reader.BaseStream.CanSeek && reader.Available < 24)
        {
            packet = default;
            time = default;
            return false;
        ...
        try
        {
            packet = reader.ReadStruct<CdgPacket>();
            time = new TimeSpan(packetNumber++ * TimeSpan.TicksPerSecond / PacketPerSecond);
            return true;
        }
