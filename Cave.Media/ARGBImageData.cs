...
namespace Cave.Media
{
	...
	public class ARGBImageData
	{
		...
		{
			if (height != data.Length * 4 / stride) throw new ArgumentOutOfRangeException();
			Data = data;
			Width = width;
			Height = height;
			Stride = stride;
		...
		{
			if (height != data.Length / stride) throw new ArgumentOutOfRangeException();
		...
		public ARGB[] GetColors(uint colorCount)
		{
			...
			for (int i = 0; i < colors.Length; i++) colors[i] = colorCounters[i].Color;
			return colors;
		}
		...
		public ARGBImageData CenterTile(int width, int height)
		{
			...
			if (l_TileX && l_TileY) throw new NotSupportedException();
