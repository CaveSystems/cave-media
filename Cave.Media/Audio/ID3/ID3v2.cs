...
/// <summary>Provides an ID3v2 tag implementation.</summary>
public sealed class ID3v2 : MP3MetaFrame
{
    ...
    string GetTextFrameText(string frameID)
    {
        foreach (var frame in frames)
        {
            ...
            {
                return textFrame.Text ?? string.Empty;
            }
        }
        return string.Empty;
    }
    ...
    internal static readonly char[] IdSplitters = [';', ',', '/', ' ', '\n', '\r'];

    internal static readonly char[] ValueSplitters = [';', ',', '/', '\n', '\r'];
    ...
    public BinaryGuid? AcousticGuid
    {
        get
        {
            if (TryGetTXXXFrame("Acoustid Id", out var acoustid) && acoustid?.Value is not null)
            {
    ...
    /// <summary>Gets the date (if present, check <see cref="HasDate"/>).</summary>
    ...
    /// <summary>Gets all frames currently present at the tag.</summary>
    ...
    /// <summary>Gets whether a date frame has been set or not.</summary>
    public bool HasDate
    {
        get
        {
    ...
    public ID3v2Header Header => header ?? throw new InvalidOperationException("Header is unset!");
    ...
    public string[] Moods => (TryGetTXXXFrame("albummood", out var mood) ? mood.Value?.Split(ValueSplitters, StringSplitOptions.RemoveEmptyEntries) : null) ?? [];
    ...
    public BinaryGuid? MusicBrainzAlbumArtistId
    {
        get
        {
            if (TryGetTXXXFrame("MusicBrainz Album Artist Id", out var frame) && frame.Value is not null)
            {
                var val = frame.Value.BeforeFirst(IdSplitters);
    ...
    public BinaryGuid? MusicBrainzAlbumId
    {
        get
        {
            if (TryGetTXXXFrame("MusicBrainz Album Id", out var frame) && frame.Value is not null)
            {
    ...
    public BinaryGuid? MusicBrainzArtistId
    {
        get
        {
            if (TryGetTXXXFrame("MusicBrainz Artist Id", out var frame) && frame.Value is not null)
            {
    ...
    public BinaryGuid? MusicBrainzReleaseGroupId
    {
        get
        {
            if (TryGetTXXXFrame("MusicBrainz Release Group Id", out var frame) && frame.Value is not null)
            {
                if (BinaryGuid.TryParse(frame.Value, out var g))
                {
                    return g;
    ...
    public int TrackCount
    {
        get
        {
            ...
            if (track != null)
            {
                ...
                if (index > 0)
                {
                    _ = int.TryParse(track[(index + 1)..], out result);
                }
            }
            return result;
        }
    ...
    public int TrackNumber
    {
        get
        {
            ...
            if (track != null)
            {
                ...
                if (index > 0)
                {
                    _ = int.TryParse(track[..index], out result);
                }
                else
                {
                    _ = int.TryParse(track, out result);
                }
            }
            return result;
        }
    ...
    /// <summary>Gets the ID3v2.x version with 2 &lt;= x &lt;= 4.</summary>
